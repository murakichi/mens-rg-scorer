...
- `src/scoring/constants.ts`: `DIFF_SCORE` に F 0.9 / G 1.1、`DIFF_VALUE`/`VALUE_DIFF` を7まで拡張。
  ...
  `SKILL_LIST` に `future: true` の技を7つ（後方伸身4回・4回半・5回ひねり、伸身前宙3回・4回ひねり、
  トリプルフル、後方3回宙返り）。`skillAllowed` / `skillOptions` / `skillOptionGroups` /
  `skillDifficulty` に `future` を追加。
...
- F・G難度の技の中身（どの技がF・Gか）は規則に無いので**こちらで決めた**。ひねり系は §3.6.2 の
  刻みを伸ばしただけで根拠があるが、2回宙返り系の2つ（トリプルフル＝F／後方3回宙返り＝G）は
  「ひねり1回ぶんを1段」と置いただけ。増やす・変えるときは `SKILL_LIST` の `future: true` の行だけで済む。
- 徒手系難度も上限まで伸ばした（5動作＝F／6動作＝G）。自動生成の投げシリーズはシェネ4回までなので
  生成では出てこない。手入力でだけ届く。
- `.mode-btn` が App のモード切替（個人/団体）と `SeriesCard` のひねり入力切替で重複している。
  今回の作業では触っていないが、E2Eでセレクタが衝突して紛らわしかった。
