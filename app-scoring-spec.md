...
- **連続投げで繋げない投げ方・受け方は作らない**
  - **その受け方から投げに繋げられないタグ**（`NO_THROW_AFTER_CATCH_TAGS` / `canThrowAfterCatch`）：
    視野外のキャッチ→視野外の投げ（物理的に実施できない）、手以外のキャッチ→連続投げ
    （足や体で受けた手具はすぐには投げられない）、手具を使ったキャッチ（押さえつけ）→連続投げ
    （押さえた状態からは投げられない）。投げが続く形（`throwsAfterCatch`＝`noViewPair`・`trailPair`）の
    直前の受けからこれらを外す（`catchStylesForPattern`）。2種類を同時に満たす受け方も含むタグで判定する。
