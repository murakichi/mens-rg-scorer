...
| F・G難度の技 | `Skill.future` が付いた `SKILL_LIST` の項目 | `futureSkillIds(future, junior)` で引ける |
...
- **技の選択肢**：`Skill.future` の技は、モードOFFでも上限を超える難度（上限FのときのG難度）でも出さない
  （`skillAllowed(id, junior, future)`）。ジュニアの2回宙返り禁止は十年後モードでも効く。
...
- **ランダム生成**：`GenerateOptions.future` を `autoTumblingTemplates` まで引き渡す。
  ...
  候補に加える（2回宙返り系は現行どおりテンプレートに出てくるときだけ）。選ばれやすさは
