...
- 縦の一回転の徒手：タッチダウンライズ・前転・後転（各1動作、`vertical`）。
...
- シェネは**横の一回転**（1動作・縦回転には数えない）。転がりも同じ。
  ...
  - 手あり／手なしで**別の技**として扱う（Q&A Q28）
  - 手ありと手なしが混在した塊は、手なし・手ありのどちらの内容とも同じ技として扱うため
    内容キーを2つ持つ（`Unit.signature` / `Unit.signatureAlt`、Q&A Q21）。
...
- 徒手難度：上位**3つ**を採用（採用候補は `adoptedHandUnits()` in `score.ts`）
  - 連続投げ（実際の投げ受けユニットが同一シリーズに2つ以上）で、そのすべてが徒手系（投げタンでない）の場合、
    その中の**A難度（間の徒手動作0）の投げ受けは採用しない**。
    例：二つ投げ→4動作→キャッチ→視野外投げ→視野外キャッチ ＝ E(0.7) のみで、動作0側の A(0.1) は付かない
  - ただし連続投げがすべてA難度の場合は、最高難度＝A を**1つだけ**採用する（0にはしない）
