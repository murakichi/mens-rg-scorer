...
- **形**（`AUTO_THROW_PATTERNS`）は7種類：
  ...
  - 投げ→キャッチ→投げ→3〜4回シェネ→キャッチ／投げ→キャッチ→投げ→1〜3回シェネ→前転→キャッチ
    （**先に最低限の投げ受けを1本置く形**＝`leadPair`。操作を足さずに投げ方を1種類増やせるので、
...
- 候補は「形 × 投げ方」を土台に作り、受け方・シェネの手（手なし＋`HANDS_TYPES` の4種類）・
  シェネの回数を**ひと回りするまで同じものを使わない順**（`cycler`）で配る。
  シェネは手の有無・手の種類で別の技として数える（Q&A Q28）ので、被らせないほど難度にも多様性にも効く。
  シェネのない形（前転3回）は手の種類を持たず、順番も消費しない
