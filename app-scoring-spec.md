...
| 手具操作加点 | `APPARATUS_OP_BONUS` | 0.1（演技全体で最大0.1） | **E難度の転回系ユニットそのもの**が（手具操作2回以上 **または** 手具を保持した技の最中に投げ）を満たすこと。同じシリーズでも別ユニットの操作では成立しない。2つのトリガーは同じ加点なので重複しない |
| 二つ投げ4動作加点 | `TWOTHROW_MOTION_BONUS` | 0.1 | 二つ投げ中に徒手4動作以上。**同じ技は重複して数えない**（§3.5.5.5(2)⑦）— 投げ〜受けの間の徒手の内訳（種類×回数、順序非依存、シェネの手の有無で区別）をキーに演技全体で1回。`notDuplicate` のシリーズはシリーズ単位でスコープを分ける |
...
- **手以外のキャッチは手具ごとに実施しやすさが違う**（`NON_HAND_CATCH_RULE`）
  ...
  - ロープは**足に絡めた手以外のキャッチで演技を締める**ことがとても多いので、
    その投げ受けで終わるシリーズを最後に置く（`nonHandLast`。並びで点数は変わらない）
