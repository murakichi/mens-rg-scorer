...
#### 現実的だが実施例の無い形（要求Dスコアで出やすさを変える）

物理的には実施できるが競技での例が無い形は、禁止せず**要求するDスコア（`minScore`）で
出やすさを変える**。`rareThrowTumblingChance(minScore)`（`autoThrows.ts`）が共通のカーブ：

| 要求値 | 確率 |
| --- | --- |
| 指定なし〜4.5（`RARE_THROW_TUMBLING_RISE_SCORE`） | 3%（`RARE_THROW_TUMBLING_BASE_CHANCE`） |
| 5.0 | 18% |
| 5.5 | 33% |
| 6.0以上 | 40%（`RARE_THROW_TUMBLING_MAX_CHANCE`） |

`HARD_THROW_FREE_SCORE`（要求値が5.0を超えるまで抑える）の裏返しで、
**高いDを求められた構成でだけ点数のために手を出す**という形にしてある。対象は3つ：

| 形 | どこ | 条件 |
| --- | --- | --- |
| 左手の視野外投げ | `AutoThrowStyle.rare`（`autoThrows.ts`） | スティックだけ（左手投げが必須投げの手具）。連続投げの1回目・2回目には使わない |
| 通常の投げタンの**背面キャッチ** | `TumblingDraws.backCatch` | 視野外のキャッチ。連続投げが続く形（視野外で受けてそのまま投げられない）と2つ同時キャッチには付けない |
| 通常の投げタンの**左手投げ** | `TumblingDraws.leftHandThrow` | スティックだけ。二つ投げとは同時に付かない |

**抽選だけでは頻度を抑えられない**。どれも技術加点（0.1）を稼ぐので、候補の3%が
生成結果の55%になった（貪欲法は点数が上がる候補を必ず採る）。そこで
`rareStyleCount` × `RARE_STYLE_WEIGHT`（0.1 ＝**稼ぐぶんと同額**、一定）で中立化し、
頻度は抽選だけで決まるようにしている。要求値で重みを変えると、重みが加点を下回る点で
一気に跳ねる（0.165で0件、0.099で全部＝0.3点の差で1/20→13/20）ので一定にしてある。
