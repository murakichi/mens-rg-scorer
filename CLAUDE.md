...
- **`score.ts`** — `computeScore(series, apparatus, opts): ScoreResult` is the single source of truth for the whole-routine **individual** score. `opts.junior` switches to the ジュニア適用規則 (§10 変更規則1): ダイビング前宙/後方宙返り半ひねり/後方伸身宙返り半ひねり are C instead of B (`JUNIOR_SKILL_DIFFICULTY`, read via `skillDifficulty(id, junior)`), the minimum throw count drops from 3 to 2 (`throwCountRequired(junior)`), 2回宙返り系 are hidden from the skill picker (`Skill.isDoubleSalto` / `skillOptions(junior)`), and throws are capped at 5 (`throwCountMax(junior)`) — the 6th onward count for nothing (difficulty, throw count, variety, required throws, technique bonus) and cost −0.3 each. The flag is threaded down to `analyzeSeries` / `calcTumblingDifficulty` and defaults to `false`. The UI calls it once (memoized) and renders the returned object. Final score = **D (難度, additive) + A (10 − deductions) + E (10 − manual execution deductions)**, both A and E floored at 0.
- **`team.ts`** — the 団体 (5-person) model and `computeTeamScore(team): TeamScoreResult`. Different data shape: a grid of `Series` × player `lanes` × `slots` of `Cell`s; horizontally-adjacent non-empty cells form a **chunk** whose difficulty reuses `SKILL_LIST`/`DIFF_*` (no throw +1, unlike individual). Team 徒手 is a different thing from individual 徒手: there is no apparatus, so the motion-count options are gone — a `motion` cell picks from the §3.6.1 table (`HAND_ELEMENTS`, 跳躍/バランス/倒立/柔軟) and scores its **団体 (5名実施)** value (`teamHandDifficulty`), never chains with neighbouring cells, and is not bumped by the 5-person rule. Because it never chains, the UI offers 徒手 only in a lane's first slot and clears the rest of that lane when one is placed (the scoring side still handles a 徒手 anywhere, so older saved data keeps working). A/E scoring here is **provisional** (`aDeduction = missing.length * 0.3`, execution not yet wired) — see the `※今後対応` note in `TeamScorer`.
