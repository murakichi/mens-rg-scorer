...
- **`team.ts`** — the 団体 (5-person) model and `computeTeamScore(team): TeamScoreResult`. Different data shape: a grid of `Series` × player `lanes` × `slots` of `Cell`s; horizontally-adjacent non-empty cells form a **chunk** whose difficulty reuses `SKILL_LIST`/`DIFF_*` (no throw +1, unlike individual). Team 徒手 is a different thing from individual 徒手: there is no apparatus, so the motion-count options are gone — a `motion` cell picks from the §3.6.1 table (`HAND_ELEMENTS`, 跳躍/バランス/倒立/柔軟) and scores its **団体 (5名実施)** value (`teamHandDifficulty`), never chains with neighbouring cells, and is not bumped by the 5-person rule. A/E scoring here is **provisional** (`aDeduction = missing.length * 0.3`, execution not yet wired) — see the `※今後対応` note in `TeamScorer`.
