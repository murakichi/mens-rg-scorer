...
- **`score.ts`** — `computeScore(series, apparatus, opts): ScoreResult` is the single source of truth for the whole-routine **individual** score. `opts.junior` switches to the ジュニア適用規則 (§10 変更規則1): ダイビング前宙/後方宙返り半ひねり are C instead of B (`JUNIOR_SKILL_DIFFICULTY`, read via `skillDifficulty(id, junior)`) and the minimum throw count drops from 3 to 2 (`throwCountRequired(junior)`). The flag is threaded down to `analyzeSeries` / `calcTumblingDifficulty` and defaults to `false`. The UI calls it once (memoized) and renders the returned object. Final score = **D (難度, additive) + A (10 − deductions) + E (10 − manual execution deductions)**, both A and E floored at 0.
