...
- **`score.ts`** — `computeScore(series, apparatus, opts): ScoreResult` is the single source of truth for the whole-routine **individual** score. `opts.junior` switches to the ジュニア適用規則 (§10 変更規則1): ダイビング前宙/後方宙返り半ひねり are C instead of B (`JUNIOR_SKILL_DIFFICULTY`, read via `skillDifficulty(id, junior)`), the minimum throw count drops from 3 to 2 (`throwCountRequired(junior)`), and throws are capped at 5 (`throwCountMax(junior)`) — the 6th onward count for nothing (difficulty, throw count, variety, required throws, technique bonus) and cost −0.3 each. The flag is threaded down to `analyzeSeries` / `calcTumblingDifficulty` and defaults to `false`. The UI calls it once (memoized) and renders the returned object. Final score = **D (難度, additive) + A (10 − deductions) + E (10 − manual execution deductions)**, both A and E floored at 0.
...
- **`seriesBreakdowns[i].handRows`** carries one row per 徒手系 unit (投げごと) with `adopted`/`inTop` flags so `SeriesCard` can strike out what doesn't count; `tumDiff`/`handDiff` sum only the top-3-adopted units, so Σ per-series = `tumblingScore`/`handScore`.
- **Single source of truth for sums:** `computeScore` computes `seriesBreakdowns` first, then the literal-sum globals (`techniqueBonus`, `apparatusOpBonus`, `twoThrowMotionBonus`, and the base of `noApparatusDeduction`) are derived by summing those breakdowns — so per-series rows and grand totals cannot drift apart. Adoption into `handScore` goes through `adoptedHandUnits()`: when a series holds two or more real throw units (投げ受け, rope-jump units excluded via `Unit.fromRopeJump`) and all of them are 徒手系 (not 投げタン), the A-difficulty ones (zero motions between throw and catch) are not adopted — per-series row and global pool alike; if every one of them is A, exactly one A survives (the group never drops to zero). On top of that, **identical content counts once for difficulty across the whole routine** (§3.4.4): every unit carries a `signature` (tumbling/投げタン = the sequence of non-A skills, so 手具を持っての前宙 and 投げての前宙 collide per Q&A Q22; hand = the composition of motions between throw and catch (type → count, order-insensitive); rope = jump id; technique tags and the presence of a throw are excluded), and among units sharing a signature only the highest-scoring one is adopted (ties keep the earlier) — but only for difficulty. Bonuses, throw counts, tumbling counts and every A-side check still see it, and `ScoreResult.unitAdopted[i][j]` tells the UI which units were skipped. Units in a `notDuplicate` series get a series-scoped key so they never collide with another series. The genuinely-global values that can't be derived per-series (top-3 adopted `tumblingScore`/`handScore`, the single `seriesBonus`, direction/throwCount/saltoChain/variety deductions) are computed across all series. When adding a rule, decide which category it is.
