...
import { JsonModal, type JsonModalMode } from "./components/JsonModal";
  ...
  return (
    <div style={S.page}>
      ...
      <header style={S.header}>
        ...
        <div style={S.ioWrap}>
          <button style={S.ioBtn} onClick={handleExport}>
            <Download size={14} /> エクスポート
          </button>
          <button style={S.ioBtn} onClick={() => fileInputRef.current?.click()}>
            <Upload size={14} /> インポート
          </button>
          <button style={S.ioBtn} onClick={openExportText}>
            テキスト出力
          </button>
          <button style={S.ioBtn} onClick={() => setJsonModalMode("import")}>
            テキスト読込
          </button>
          <input
            ref={fileInputRef}
      ...
      <section style={S.card}>
        ...
        <div style={S.appWrap}>
          {(Object.entries(APPARATUS) as [ApparatusKey, { name: string }][]).map(([k, v]) => (
            <button key={k} style={k === apparatus ? S.appActive : S.appBtn} onClick={() => setApparatus(k)}>
              {v.name}
            </button>
          ))}
        </div>
      ...
      <button style={S.addBtn} onClick={addSeries}>
