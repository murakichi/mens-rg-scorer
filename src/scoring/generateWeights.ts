...
/**
 * **実施例の無い投げ受け**1つあたりの重み（`rareStyleCount`）。
 * 左手の視野外投げ・投げタンの背面キャッチ・投げタンの左手投げが対象。
 * 候補を出す確率（`rareThrowTumblingChance`）だけでは頻度を抑えられない：
 * どれも技術加点（0.1）を稼ぐので、候補にあれば貪欲法が**進んで**採ってしまい、
 * 候補の3%が生成結果の55%になった。
 * そこで**稼ぐぶんと同額**の重みを一定で掛け、進んで狙いには行かない状態にする。
 * こうすると頻度は抽選だけで決まり、要求するDスコアに応じて滑らかに上がる
 * （要求値で重みを変えると、重みが加点を下回る点で一気に跳ねてしまう）。
 */
export const RARE_STYLE_WEIGHT = 0.1;
