...
## 追記（2026-09-13）：最新 main へリベース後の再計測

本レポート冒頭の数値は分岐点（`eb04d60`）時点のもの。その後 main が73コミット進み、
自動生成系の新モジュール（`generate.ts` / `autoTumblings.ts` / `autoThrows.ts` / `templates.ts`）と
そのテストが入ったため、母数ごと変わっている。最新 main（`ece0b1e`）をマージして再計測した。

| | main のみ（410件） | main + 本ブランチ（511件+4skip） |
|---|---|---|
| Statements | 92.48% | **93.81%** |
| Branches | 87.71% | **89.12%** |
| Functions | 92.19% | **93.18%** |

ファイル別（Stmts / Branch）：

| ファイル | main のみ | マージ後 |
|---|---|---|
| `share.ts` | 0 / 0 | **100 / 100** |
| `analysis.ts` | 96.82 / 93.46 | **98.84 / 96.07** |
| `score.ts` | 98.87 / 97.18 | **100 / 98.59** |
| `constants.ts` | 97.58 / 94.57 | **98.55 / 94.57** |
| `autoThrows.ts` | 100 / 100 | 100 / 100 |
| `autoTumblings.ts` | 98.82 / 94.13 | 98.82 / 94.13 |
| `generate.ts` | 95.27 / 93.77 | 95.27 / 93.77 |
| `templates.ts` | 87.61 / 82.11 | 87.61 / 82.11 |
| `team.ts` | 77.67 / 64.07 | 77.67 / 64.07 |

- main 側の新モジュールはいずれもテスト付きで入っており、個人モードの採点コアも main の追加分で
  底上げされていた（`score.ts` は分岐点時点の 97.12/94.22 から 98.87/97.18 に）。
- **`team.ts` は数値が1桁も動いていない**＝この73コミットで団体には一切触れられていない。
  依然として最大の穴（Branch 64.07%）。次点は新規の `templates.ts`（Branch 82.11%）。
- マージ時、`unit.test.ts` の `motionTimes` のテストが1件落ちた。main で仕様が変わり
  （`42f8c90`）、未指定は1回のままだが **0を入れたら0回** として扱う（0回の動作は難度にも
  構成にも数えない）ようになったため。旧仕様を書いていた側が誤りなのでテストを更新し、
  「0回の徒手動作は動作数に数えない」ケースを追加した。
- issue #3 / #6 / #23 は最新 main でも同じ値で再現することを確認済み（skip テストは有効なまま）。
